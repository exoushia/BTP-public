...
if __name__ == '__main__':
	...
	if args.mode == "train_&_test":
		...
		if args.to_train_split :
			...
			del df_train, input_ids_tr , attention_ids_tr , sent_ids_tr
		...
		loss_values, val_accuracy, model = train_model_custom_bert(config_2.mode,train_loader,validation_dataloader,config,total_steps,args.path_to_ckpt)
		...
		if config_2.mode == 'lstm':
			y_pred, y_true = evaluate(model,test_loader,'Test')
		else :
			y_pred, y_true = evaluate2(model,test_loader,'Test')
