...
def assign_class(df,col1="answers",col2="is_answerable",dict_num_classes,list_keywords):
    classes = []
    labels = []
...
def sample_from_class(df,sample_ratio_list,label):
    df = pd.concat([ df.loc[df['label']==label[0]].sample(sample_ratio_list[0]) , df.loc[df['label']==label[1]].sample(sample_ratio_list[1]) , df.loc[df['label']==label[2]].sample(sample_ratio_list[2])]) 
    return df
